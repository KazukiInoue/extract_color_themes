...
void k_means_color_subtraction(cv::Mat src, cv::Mat &dst, const int clusterNum,vector<vector<double>> &clusterInfo) {
	...
	cout << clusterInfo.front().size() << endl;

	cv::imwrite("subtract_image.jpg", dst);
}